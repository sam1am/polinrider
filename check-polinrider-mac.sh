...
found=0
while IFS= read -r f; do
  grep -qE '^\.gitignore$' "$f" 2>/dev/null && { hit "$f lists .gitignore (PolinRider artifact)"; found=1; }
  grep -qE '^temp_auto_push\.bat$' "$f" 2>/dev/null && { hit "$f lists temp_auto_push.bat"; found=1; }
done < <(grep -E '/\.gitignore$' "$INV" 2>/dev/null)
[ "$found" = 0 ] && ok "no PolinRider .gitignore artifact lines"
